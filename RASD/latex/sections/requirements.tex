...
more generally to a great number of request throughout the day. Basing the analysis on
competitors' results an average of 200000 (two hundred thousands) subscribed users and
10000 (ten thousands) reservations per day may have to be expected and so correctly managed.
...
\subsection{Software System Attributes} 
	...
	The StB must guarantees a 3-nines availability (99.9 percent) with a downtime not greater than 8 hours per year.
	\subsubsection{Security}
User credentials and payment information will be stored. Data confidentiality is a primary concern.
In addition, stored information must be sent to affiliated  transport Company systems or shared vehicle Company  systems when the user wants to buy tickets or rent a shared vehicle. To ensure the security of this information, the communication protocol between Systems must use asymmetric keys and digital signatures.
	
	\subsubsection{Maintainability}
	The StB must be designed in the way to easly correct defects or their cause,
	...
	The StB must be able to run in all main mobile OS(Android, iOS, Nokia OS, blackBerry OS, windows-phone OS) and being supported by all the main Web Browser(Google Crome, Safari,FireFox, Microsoft Edge, Internet Explorer, ...)\newline

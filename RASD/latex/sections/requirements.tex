...
\subsection{Functional Requirements}
	...
	\begin{enumerate}[resume, label={[R\arabic*]}]
		\item \mylabel{R-change-preferences}{for each type of preference, the S2B must provide the User the possibility to set or change the value(s)}
		\item \mylabel{R-store-preferences}{for each type of preference, the S2B must store the preference value(s)}
	\end{enumerate}
	...
	\goal{G-locate}
	\begin{enumerate}[resume, label={[R\arabic*]}]
		\item \mylabel{R-provide-loc-service}{when a user visualize an incoming appointment for which a shared vehicle of an affiliated company has to be used, the S2B provides a localization service}
	\end{enumerate}
	\begin{itemize}
